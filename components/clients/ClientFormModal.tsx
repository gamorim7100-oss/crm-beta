...
export function ClientFormModal({ onClose, onSaved }: ClientFormModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-bg-card border border-white/[0.06] rounded-xl w-full max-w-2xl p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-bold text-white mb-4">Novo Cliente</h2>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <h3 className="text-sm font-semibold text-gray-300 mb-3">Identificação</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-bg-secondary border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-400"
                  required
                />
              </div>
              <div>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-300 mb-3">Dados do Consórcio</h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">Valor do crédito *</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">Cota</label>
                <input
                  type="number"
                  value={formData.cota}
              ...
              <div className="col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-bg-secondary border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-400 [color-scheme:dark]"
                  required
                />
              </div>
              <div className="col-span-2">
                <label className="block text-xs text-gray-400 mb-1">Critério de Lance</label>
                <textarea
                  value={formData.criterio_de_lance}
                  onChange={(e) => setFormData({ ...formData, criterio_de_lance: e.target.value })}
                  className="w-full bg-bg-secondary border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-400 h-20 resize-none"
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-gray-400 hover:text-white transition-colors text-sm border border-white/10 rounded-lg"
            >
              Cancelar
            </button>
            <button
