    ...
    <div className="bg-bg-card border border-border rounded-xl p-5 shadow-[0_4px_24px_rgba(0,0,0,0.3)] col-span-2">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-white">Vendas por Tipo</h3>
        </div>
        <select
          value={selectedMonth}
          onChange={(e) => setSelectedMonth(e.target.value)}
          className="bg-bg-secondary text-gray-300 text-xs border border-white/10 rounded-lg px-3 py-1.5 focus:outline-none focus:border-emerald-400"
        >
          ...
          <div className="col-span-2 grid grid-cols-3 gap-2 mt-2">
            {data.map((item) => (
              <div key={item.name} className="bg-white/5 rounded-lg p-3 text-center">
                ...
                <p className="text-lg font-bold text-white">{item.qty}</p>
