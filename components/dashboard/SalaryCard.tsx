      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-1">
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">Salário Previsto</p>
            <div className="group relative">
              <Info size={12} className="text-gray-500 cursor-help" />
              <div className="absolute bottom-full left-0 mb-2 w-48 bg-bg-secondary border border-white/10 rounded-lg p-2 text-xs text-gray-400 hidden group-hover:block z-10">
          ...
          <p className="text-xs text-gray-500 mt-0.5">(previsto)</p>
        </div>
      ...
      {salary && salary.grossCommission > 0 && (
        <div className="mt-3 pt-3 border-t border-white/[0.06]">
          <p className="text-xs text-gray-500">
            Comissão bruta: <span className="text-gray-300">{formatCurrency(salary.grossCommission)}</span>
          </p>
        </div>
      )}
    </div>
