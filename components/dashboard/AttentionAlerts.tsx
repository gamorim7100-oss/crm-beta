  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl p-5 shadow-[0_4px_24px_rgba(0,0,0,0.3)]">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold text-white">Atenção</h3>
      ...
      <div className="space-y-2">
        {alerts.map((alert) => (
          <div key={alert.reason} className="border border-white/[0.06] rounded-lg overflow-hidden">
            <button
            ...
            {expanded === alert.reason && (
              <div className="px-3 pb-2 space-y-1">
                {alert.clients.map((client) => (
                  <div key={client.id} className="flex items-center gap-2 text-xs text-gray-400 py-1 px-2 rounded bg-white/5">
