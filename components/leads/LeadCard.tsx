...
export function LeadCard({ lead, onClick, onConvert, onDelete }: LeadCardProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      onClick={onClick}
      className="bg-bg-card border border-white/[0.06] rounded-lg p-3 cursor-grab active:cursor-grabbing hover:border-white/[0.15] transition-all shadow-[0_2px_8px_rgba(0,0,0,0.2)] relative group"
    >
      <div className="flex items-start gap-3">
        ...
        {onDelete && (
          <button
            ...
            className="absolute top-2 right-2 text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1 rounded-lg hover:bg-red-500/10"
          >
            <Trash2 size={14} />
          </button>
        )}
